...
export default function SurveyPublicPage() {
  ...
  const [status, setStatus] = useState('loading') // loading | invalid | expired | completed | intro | survey | submitting
  const [currentQ, setCurrentQ] = useState(0)
  const [answers, setAnswers] = useState({})
  const [introData, setIntroData] = useState({ name: '', dob: '', job: '', smoker: false })
  ...
  async function validateLink() {
    ...
    setLink(data)
    setIntroData({ name: data.prospect_name, dob: data.prospect_dob, job: data.prospect_job, smoker: data.prospect_smoker })

    // Mark as opened (start 1 hour timer from first open)
    if (!data.opened_at) {
  ...
  function handleAnswer(questionId, optionId) {
    ...
    if (next < QUESTIONS.length) {
      setTimeout(() => setCurrentQ(next), 300)
    } else {
  ...
  async function submitSurvey(finalAnswers) {
    ...
    const result = calculateScore(finalAnswers, introData.dob, introData.smoker)
    ...
    navigate(`/s/${token}/done`, { state: { result, name: introData.name } })
  }
  ...
  if (status === 'intro') {
    return (
      <Screen>
        <div style={s.card}>
          <div style={s.kdBrand}>KenalDiri</div>
          <h1 style={s.introTitle}>Halo, {introData.name}!</h1>
          <p style={s.introText}>
            Kamu akan menjawab <strong>25 pertanyaan</strong> singkat tentang kepribadian dan gaya hidupmu.
          ...
          <div style={s.privacyNote}>
            Data yang kamu isi hanya digunakan untuk keperluan analisa kepribadian finansial dan tidak dibagikan kepada pihak ketiga.
          </div>
          <button onClick={() => setStatus('survey')} style={s.startBtn}>Mulai Sekarang →</button>
        </div>
      </Screen>
    )
  }
  ...
  return (
    <Screen>
      <div style={s.surveyWrap}>
        ...
        {/* Phase label */}
        <div style={s.phaseLabel}>Bagian {q.phase}</div>
        ...
        <div style={s.questionCard}>
          ...
          <div style={s.options}>
            {q.options.map(opt => (
              ...
              >
                <span style={s.optionLetter}>{opt.id.toUpperCase()}</span>
        ...
        <div style={s.brandFooter}>KenalDiri · Analisa Kepribadian Finansial</div>
      </div>
    </Screen>
  )
}
...
const s = {
  ...
  kdBrand: { fontSize:'13px', fontWeight:'700', color:'#7F77DD', letterSpacing:'0.1em', marginBottom:'20px', textTransform:'uppercase' },
  ...
  startBtn: { width:'100%', padding:'14px', background:'#7F77DD', color:'#fff', border:'none', borderRadius:'10px', fontSize:'15px', fontWeight:'600', cursor:'pointer' },
  ...
  progressFill: { height:'100%', background:'#7F77DD', borderRadius:'99px', transition:'width 0.3s' },
  progressLabel: { fontSize:'12px', color:'#999', minWidth:'40px', textAlign:'right' },
  phaseLabel: { fontSize:'12px', color:'#7F77DD', fontWeight:'500', textTransform:'uppercase', letterSpacing:'0.08em' },
  ...
  optionLetter: { width:'28px', height:'28px', background:'#e8e8f0', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px', fontWeight:'700', color:'#7F77DD', flexShrink:0 },
